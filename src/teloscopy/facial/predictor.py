...
_POPULATION_SNP_FREQUENCIES: dict[str, dict[str, list]] = _load_json(
    "population_snp_frequencies.json"
)

# Risk allele and reference allele for each SNP.
# Sources: ClinVar, dbSNP, published GWAS.
_SNP_ALLELES: dict[str, list[str]] = _load_json("snp_alleles.json")
...
_SNP_GENOMIC_CONTEXT: dict[str, list] = _load_json("snp_genomic_context.json")
...
_PHARMACOGENOMIC_MAP: dict[str, dict] = _load_json("pharmacogenomic_map.json")
...
_MTDNA_HAPLOGROUP_PRIORS: dict[str, dict[str, float]] = _load_json(
    "mtdna_haplogroup_priors.json"
)
